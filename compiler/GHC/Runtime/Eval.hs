...
findMatchingInstances ty = do
  ...
  dflags <- getDynFlags
  return $ concatMap (\cls -> do
    let (matches, _, _) = lookupInstEnv True ies cls [ty]
    filter (checkInstKind dflags ty . fst) matches) allClasses

{-
  Check that a class instance is actually well-kinded for the search type.
  It's possible for us to get ill-kinded instance matches from @lookupInstEnv@,
  so we check that they are valid by running the linter over them after matching.
-}
checkInstKind :: DynFlags -> Type -> ClsInst -> Bool
checkInstKind dflags queryTy clsInst =
  let cls = mkClassPred (is_cls clsInst) [queryTy]
      fvs = tyCoVarsOfTypeWellScoped cls
      tcErr = lintTypes dflags fvs [cls]
  in isNothing tcErr
